...
export default function KullanicilarPage() {
  ...
  function linkKopyala(musteriId: string) {
    const link = `${window.location.origin}/musteri/${musteriId}`;
    navigator.clipboard.writeText(link);
    setKopyalanan(musteriId);
    setTimeout(() => setKopyalanan(null), 2000);
  }

  function musteriToggle(musteriId: string) {
    setForm(prev => ({
      ...prev,
      musteri_ids: prev.musteri_ids.includes(musteriId)
        ? prev.musteri_ids.filter(id => id !== musteriId)
        : [...prev.musteri_ids, musteriId],
    }));
  }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-gray-500 mb-5">
              <strong>{kullanicilar.find(k => k.id === silmeOnay)?.email}</strong> kullanıcısı silinecek.
            </p>
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-100">
        <div className="px-8 py-4">
          ...
          <p className="text-xs text-gray-400">Müşteri kullanıcılarını oluşturun ve panel erişim linklerini paylaşın</p>
        </div>
      </header>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden sticky top-24">
            ...
            <form onSubmit={kullaniciOlustur} className="p-6 space-y-4">
              ...
              {[
                { key: "ad", label: "Ad Soyad", type: "text", placeholder: "Filko Yöneticisi" },
              ...
              <button
                type="submit"
                disabled={kaydediyor || form.musteri_ids.length === 0}
                className="w-full py-2.5 bg-nabiz-navy text-white font-semibold rounded-xl text-sm hover:opacity-90 transition-opacity disabled:opacity-50"
              >
            ...
            <div className="divide-y divide-gray-50">
              {kullanicilar.map(k => {
                const musteriAdlari = (k.musteri_ids || [])
                  .map(mid => musteriler.find(m => m.id === mid)?.musteriAd || mid)
                  .join(", ");
                return (
