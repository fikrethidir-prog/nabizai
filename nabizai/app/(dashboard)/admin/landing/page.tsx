...
const settingGroups = [
  {
    ...
    fields: [
      ...
      { key: "hero_desc", label: "Açıklama", placeholder: "Yerel haberler, sosyal medya...", multiline: true },
    ],
  },
  {
    title: "İstatistikler",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "pkg_1_price", label: "Paket 1 — Fiyat", placeholder: "15.000 ₺" },
      { key: "pkg_2_name", label: "Paket 2 — Ad", placeholder: "Radar Pro" },
      { key: "pkg_2_price", label: "Paket 2 — Fiyat", placeholder: "35.000 ₺" },
      { key: "pkg_3_name", label: "Paket 3 — Ad", placeholder: "İstihbarat" },
      { key: "pkg_3_price", label: "Paket 3 — Fiyat", placeholder: "75.000+ ₺" },
    ],
  },
];
...
export default function AdminLandingPage() {
  ...
  async function loadSettings() {
    ...
    try {
      ...
      if (fetchError) {
        setError("Ayarlar yüklenemedi: " + fetchError.message);
        return;
      }
  ...
  async function handleSave() {
    ...
    try {
      // Her bir değişen ayarı güncelle
      ...
      for (const [key, value] of updates) {
        ...
        if (upsertError) {
          setError(`"${key}" güncellenemedi: ${upsertError.message}`);
          return;
        }
      }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-nabiz-dark">Landing Page CMS</h1>
              <p className="text-xs text-gray-400">nabizai.com içeriğini buradan yönetin</p>
            </div>
          </div>
            ...
            <button
              ...
              className={`px-5 py-2 text-sm font-semibold rounded-xl transition-all duration-300 flex items-center gap-2 ${
                saved
                  ...
                  : hasChanges
                  ? "bg-nabiz-orange text-white shadow-lg shadow-nabiz-orange/20 hover:shadow-xl hover:shadow-nabiz-orange/30"
      ...
      {error && (
        <div className="max-w-5xl mx-auto px-6 mt-4">
          <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-sm text-nabiz-red">
            {error}
          </div>
        </div>
      )}
                ...
                <div key={field.key} className={field.multiline ? "md:col-span-2" : ""}>
                  ...
                  {field.multiline ? (
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-nabiz-dark placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-nabiz-navy/20 focus:border-nabiz-navy transition-all duration-200 resize-none"
                    />
                  ) : (
                    <input
                      type="text"
                      ...
                      onChange={(e) => handleChange(field.key, e.target.value)}
                      placeholder={field.placeholder}
                      className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-nabiz-dark placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-nabiz-navy/20 focus:border-nabiz-navy transition-all duration-200 ${
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 leading-relaxed">
                Burada yaptığınız değişiklikler Supabase <code className="px-1.5 py-0.5 bg-white rounded text-nabiz-navy text-xs font-mono">site_settings</code> tablosuna kaydedilir.
                Landing page bu tabloyu okur ve içeriği otomatik günceller (1 saat cache).
                Yeni müşteri eklendiğinde sadece değerleri değiştirin — kod dokunmanıza gerek yok.
              </p>
            </div>
          </div>
        </div>
